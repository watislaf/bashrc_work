...
function bash__gTools__ {
  ...
  function BASH_GTOOLS__PRINT__COMMAND { 
	...
	if [[ $((BASH_GTOOLS__VAR_INCREMENT_EACH_TIME_PRINT_IS_CALLED % 2)) == 0 ]]; then 
		BASH_GTOOLS__VAR_LINE='${BASH_GTOOLS__START_COLOR}*-*-*-*-*-*-*-*-*-*-*>'
	else 
		BASH_GTOOLS__VAR_LINE='${BASH_GTOOLS__START_COLOR}-.-.-.-.-.-.-.-.-.-.->${BASH_GTOOLS__START_COLOR}'
	fi;
	printf "    %s ${BASH_GTOOLS__START_COLOR}%s${BASH_GTOOLS__START_COLOR} $BASH_GTOOLS__VAR_COMMAND \n" "$BASH_GTOOLS__VAR_HELP" "${BASH_GTOOLS__VAR_LINE:${#BASH_GTOOLS__VAR_HELP}}"
  }
